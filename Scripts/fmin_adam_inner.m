function [x, fval, exitflag, output] = fmin_adam_inner(fun, x0, stepSize, beta1, beta2, epsilon, nEpochSize, options)
...
if (~exist('options', 'var') || isempty(options))
   options = optimset(@fmin_adam_inner);
end
...
if (~exist('nEpochSize', 'var') || isempty(nEpochSize))
   nEpochSize = 1;% numberofvariables;
end
...
%          figure,plot(vfCost);
%          xHist_i = reshape(xHist,[128,128,3,nIter+2]);
%          figure;
%          subplot(321),imshow3(xHist_i(:,:,1:2,2),[],[1,2]);
%          subplot(322),imshow(1./abs(log(xHist_i(:,:,3,2))/0.02),[0,0.2]);colorbar;
%          subplot(323),imshow3(xHist_i(:,:,1:2,round(nIter/2)),[],[1,2]);
%          subplot(324),imshow(1./abs(log(xHist_i(:,:,3,round(nIter/2)))/0.02),[0,0.2]);colorbar;
%          subplot(325),imshow3(xHist_i(:,:,1:2,nIter+1),[],[1,2]);
%          subplot(326),imshow(1./abs(log(xHist_i(:,:,3,nIter+1))/0.02),[0,0.2]);colorbar;
